...
- Hotfix 2026-08-31 (post-Sprint 23), 2 parts:
  ...
  - 392 vitest (+3), build + lint clean, both merged to `main` (CI green). If "everything 500s": hit `/api/health` — `database:false` = bad `DATABASE_URL`; `migrations:false` = run `prisma migrate deploy`; a 500 on `/api/health` itself = read the app log for `[env] FATAL`.
