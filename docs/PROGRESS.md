...
- Branches: integration branch `develop` / `main`. develop→main merges (user-authorised, per sprint): … → Sprint 18 merge `1e0130b` (#23) → Sprint 19 merge (#24) → Sprint 20 merge `6377fe2` (#25) → Sprint 21 merge `3fa8bd7` (#26) → Sprint 22 merge `98b5c0a` (#20) → Sprint 23 merge `8c86110` (#27). No tag yet (tag at Phase 7 / first real deploy).
...
- Hotfix 2026-08-31 (post-Sprint 23), 2 parts:
  - **L-007** — public marketing pages (`/`, `/servicios/*`, `/zonas`, `/guias`) 500'd when Postgres was unreachable (unguarded `await service.<dbCall>()` for optional sections). Added `src/lib/safe.ts`; wrapped every optional DB read → each public route now returns 200 (degraded) instead of 500.
  - **L-008** — `src/lib/env.ts` threw at boot for adapter-config gaps (`WHATSAPP_BUSINESS_NUMBER` with the default `link` adapter, SMTP, S3, Claude) → a fresh Hostinger+Supabase deploy 500'd on every route incl. `/api/health`. Now only `DATABASE_URL` + (prod) `AUTH_SECRET`/`SIGNED_LINK_SECRET` are FATAL; the rest are `[env]` warnings and the app boots. `/api/health` now also reports `checks.migrations` + a `detail` string. `docs/deploy-hostinger.md` got a Supabase section + ISE troubleshooting.
  - **L-009** — after setting `NODE_ENV=production` on the host, the build broke: `next build` needs `typescript`/`@types/*`/`tailwindcss`/`@tailwindcss/postcss` but they were `devDependencies` and a production install skips those. Moved them + `prisma` + `tsx` to `dependencies`; added `postinstall: prisma generate`. Verified a production-only install now builds.
  - 392 vitest, build + lint clean, all merged to `main` (CI green). If "everything 500s": hit `/api/health` — `database:false` = bad `DATABASE_URL`; `migrations:false` = run `prisma migrate deploy`; a 500 on `/api/health` itself = read the app log for `[env] FATAL`. If "no compila": Hostinger did a prod-only install — everything build needs is now in `dependencies`, just redeploy.
  - **L-010** — Hostinger build kept failing regardless (likely RAM). Switched to `output: "standalone"` + a `deploy-bundle.yml` workflow (`workflow_dispatch`, input `APP_URL`) that builds in CI and uploads `deploy-bundle.tgz`. Operator downloads it, extracts on Hostinger, sets 5 env vars, start command `server.js` — no install/build on the host. `docs/deploy-hostinger.md` rewritten around this.
  - Live domain (operator): `https://lightsteelblue-seahorse-671905.hostingersite.com` — DB on Supabase. Deploy path is now: run the "Deploy bundle" GitHub Action → download artifact → upload to Hostinger → run `prisma migrate deploy` once against the Supabase **direct** string → start. `docs/deploy-hostinger.md` has the full runbook.
