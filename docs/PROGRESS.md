...
- Branches: integration branch `develop` / `main`. develop→main merges (user-authorised, per sprint): … → Sprint 15 merge `7b35ebe` (#19) → Sprint 16 merge (#22). No tag yet (tag at Phase 7 / first real deploy).
...
| 5 Development | MVP core done (#2–#19, #28, #29) + growth #22 (Sprint 16). Next: #21 (Sprint 17). | docs/sprints/, docs/05-test-points.md |
| 6 Documentation | pending | docs/architecture.md, docs/api/ |
| 7 Release | pending | docs/07-release.md |
| 8 Website | n/a | — |

## Current position
- Phase: 5 — Development. MVP core (#2–#19, #28, #29) + growth #22 done + verified. Awaiting user close on GitHub (#1 EPIC closes when the user closes all).
...
- Next action: **growth backlog** order #22 ✅ → **#21 (Sprint 17)** → #23 → #24 → #25 → #26 → #20 → #27. Separately, before go-live: definitive legal texts + DPIA (release gate), and the actual Hostinger deploy (`docs/deploy-hostinger.md` is final; needs operator hPanel access + the real secrets; `APP_URL` + `NEXT_PUBLIC_*` at BUILD time).
...
- Forge issues in progress: see docs/issues.md — #2–#19, #22, #28, #29 resolved (awaiting user close); #20, #21, #23–#27 (growth) open; building in dependency order
...
Last updated: 2026-08-31 — Phase 5; Sprint 16 (#22 verified professional network) closed, merged to main; next: Sprint 17 = #21 (Carta de Confianza)
