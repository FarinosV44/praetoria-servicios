...
- Hotfix 2026-08-31 (post-Sprint 23) — **L-007**: public marketing pages (`/`, `/servicios/*`, `/zonas`, `/guias`) returned a 500 whenever Postgres was unreachable (unguarded `await service.<dbCall>()` for optional sections). Added `src/lib/safe.ts` and wrapped every optional DB read on the public pages; with the DB down each route now returns 200 (degraded) instead of 500. 392 vitest (+3), build clean. If "everything 500s": check Postgres is up (`docker compose up -d`) AND `.env` is valid (`src/lib/env.ts` fails fast on bad config).
